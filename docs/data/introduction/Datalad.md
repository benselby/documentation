...
If you tried this at home, you won't need me to tell me that they will have all failed with stderr of "Permission denied". This is because, of course, DataLad locks down the files it's saved. In order to do this and totally bork our `T1w`s, we'll need to unlock them. There are multiple ways of doing this, but we'll focus on only two salient ones: **`datalad unlock`** and **`datalad run`**.
...
Doing so will ensure that only small text files are placed directly in the repo, ensuring against massive, potentially very data-intensive `datalad install` operations.
